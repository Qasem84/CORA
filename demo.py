...
def main(args):
    ...
    # Custom class prompts (open vocabulary)
    if args.custom_classes is not None:
        print(f"Using custom categories: {args.custom_classes}")
        category_list = args.custom_classes
        id2name = {i: name for i, name in enumerate(category_list)}
        dataset_val.label2catid = {i: i for i in range(len(category_list))}
    else:
        ...
        if hasattr(dataset_val, 'coco'):
            ...
            # Now correctly assign label2catid based on category_list order
    ...
    with torch.no_grad():
        for idx in range(2880):  # change range or add --image_idx for customization
            ...
            outputs = model(img, categories=category_list)
